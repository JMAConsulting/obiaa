...
spl_autoload_register('_mjwshared_civix_class_loader', TRUE, TRUE);

function _mjwshared_civix_class_loader($class) {
  if ($class === 'CRM_Mjwshared_DAO_Base') {
    if (version_compare(CRM_Utils_System::version(), '5.74.beta', '>=')) {
      class_alias('CRM_Core_DAO_Base', 'CRM_Mjwshared_DAO_Base');
      // ^^ Materialize concrete names -- encourage IDE's to pick up on this association.
    }
    else {
      $realClass = 'CiviMix\\Schema\\Mjwshared\\DAO';
  ...
  if (strpos($class, 'CiviMix\\Schema\\Mjwshared\\') === 0) {
 ...
 */
function _mjwshared_civix_civicrm_enable(): void {
  _mjwshared_civix_civicrm_config();
  // Based on <compatibility>, this does not currently require mixin/polyfill.php.
}
