 ...
 * extension.
 */
class CRM_Sweetalert_ExtensionUtil {
  const SHORT_NAME = 'sweetalert';
  const LONG_NAME = 'sweetalert';
  const CLASS_PREFIX = 'CRM_Sweetalert';
 ...
 */
function _sweetalert_civix_civicrm_config($config = NULL) {
 ...
 * @return bool
 */
function _sweetalert_civix_insert_navigation_menu(&$menu, $path, $item) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _sweetalert_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
 ...
 */
function _sweetalert_civix_navigationMenu(&$nodes) {
  if (!is_callable(['CRM_Core_BAO_Navigation', 'fixNavigationMenu'])) {
    _sweetalert_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _sweetalert_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _sweetalert_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _sweetalert_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _sweetalert_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
