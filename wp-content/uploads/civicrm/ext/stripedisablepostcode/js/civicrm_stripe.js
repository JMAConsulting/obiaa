...
(function($, ts) {
  ...
  var script = {
    ...
    createElementCard: function(stripeElements) {
      ...
      var profilePostCodeElement = document.getElementById('postal_code-' + CRM.vars[script.name].profilePostCodeFieldId);
      if (profilePostCodeElement && !postCodeElement) {
        var postCode = document.getElementById('postal_code-' + CRM.vars[script.name].profilePostCodeFieldId).value;
        script.debugging('existing postcode: ' + postCode);
        elementsCreateParams.value.postalCode = postCode;
      }

      // Override to disable postcode collection in stripe.
      elementsCreateParams.hidePostalCode = true;
