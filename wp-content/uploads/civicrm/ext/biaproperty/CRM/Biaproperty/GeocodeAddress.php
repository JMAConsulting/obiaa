 ...
 * issues
 */
class CRM_Biaproperty_GeocodeAddress {
  ...
  public function run() {
    ...
    // do check for parse street address.
    return $this->processUnits($processGeocode, $parseStreetAddress);
  }
   ...
   * @throws Exception
   */
  public function processUnits($processGeocode, $parseStreetAddress) {
    // build where clause.
    $clause = [];
    $params = [];
    if ($this->start) {
    ...
    while ($dao->fetch()) {
      ...
      if ($parseStreetAddress) {
        ...
        elseif ($dao->street_address) {
          ...
          $url = CRM_Utils_System::url('civicrm/unit-address', "reset=1&id={$dao->id}");
          $unparseableUnitAddress[] = " Unit ID: " . $dao->id . " <a href =\"$url\"> " . $dao->street_address . " </a> ";
    ...
    if ($parseStreetAddress) {
      ...
      if ($unparseableUnitAddress) {
        $this->returnMessages[] = "<br />\n" . ts("Following is the list of units whose address is not parsed:") . "<br />\n";
        foreach ($unparseableUnitAddress as $contactLink) {
