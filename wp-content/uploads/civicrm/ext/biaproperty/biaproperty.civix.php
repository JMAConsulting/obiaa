...
spl_autoload_register('_biaproperty_civix_class_loader', TRUE, TRUE);

function _biaproperty_civix_class_loader($class) {
  if ($class === 'CRM_Biaproperty_DAO_Base') {
    if (version_compare(CRM_Utils_System::version(), '5.74.beta', '>=')) {
      class_alias('CRM_Core_DAO_Base', 'CRM_Biaproperty_DAO_Base');
      // ^^ Materialize concrete names -- encourage IDE's to pick up on this association.
    }
    else {
      $realClass = 'CiviMix\\Schema\\Biaproperty\\DAO';
  ...
  if (strpos($class, 'CiviMix\\Schema\\Biaproperty\\') === 0) {
