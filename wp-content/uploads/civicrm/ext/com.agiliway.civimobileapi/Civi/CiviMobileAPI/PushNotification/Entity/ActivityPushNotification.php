...
class ActivityPushNotification extends BasePushNotification {
  ...
  function __construct($action, $entity, $id, $entityInstance) {
    ...
    $this->actionText = [
      ...
      'deleteInCase' => '%display_name ' . E::ts('has deleted activity in case.'),
    ];
  }
  ...
  public function handlePostHook() {
    ...
    if (CRM_CiviMobileAPI_Utils_Activity::getCaseIdByActivity($this->id)) {
      return;
    }
  ...
  public function handlePreHook() {
    ...
    $caseId = CRM_CiviMobileAPI_Utils_Activity::getCaseIdByActivity($this->id);

    $contacts = $this->getContacts();
    $title = $this->getTitle();

    if ($caseId) {
      $this->action = 'deleteInCase';
      $this->entity = 'Case';
      $this->id = $caseId;
    }
     ...
     * Delete action
     */
    if ($this->action == 'delete' || $this->action == 'deleteInCase') {
      if (!$caseId) {
        $data['entity'] .= 'Delete';
      }
