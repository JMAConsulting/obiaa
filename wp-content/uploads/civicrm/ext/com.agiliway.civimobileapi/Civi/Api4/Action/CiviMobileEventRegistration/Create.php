...
class Create extends BasicCreateAction {
  ...
  public function _run(Result $result) {
    ...
    if (isset($params['event_id'])) {
      ...
      throw new CRM_Core_Exception(E::ts("Field 'event_id' is required"));
    }
    ...
    if ($isEventAvailable) {
      if (!$hasRegistrationOnEvent) {
        ...
        try {
          ...
          throw new CRM_Core_Exception(E::ts('Participant registration error.'));
        }
      } else {
        throw new CRM_Core_Exception(E::ts('You already registered on Event.'));
      }
    } else {
      throw new CRM_Core_Exception(E::ts('The event is currently full.'));
    }
  ...
  private function ensureRequiredFieldsExist($requiredProfileFields, $params) {
    foreach ($requiredProfileFields as $field) {
      if (empty($params[$field])) {
        throw new CRM_Core_Exception(E::ts("Field '{$field}' is required."));
      }
    }
  }
    ...
    foreach ($params as $paramName => $paramValue) {
      if (strpos($paramName, 'custom_') === 0) {
        [
          $customFieldID,
          $customValueID,
        ] = CRM_Core_BAO_CustomField::getKeyID($paramName, TRUE);
  ...
  private function updateContactFields($params, $fields, $contactId) {
    ...
    foreach ($params as $key => $value) {
      if (isset($fields[$key]) && in_array($fields[$key]['field_type'], [
          'Individual',
          'Contact',
        ])) {
