  ...
  public function _run(Result $result) {
    ...
    if ($isFileUploaded) {
      ...
      if ($attachmentCount >= $maxNumAttachments) {
        throw new CRM_Core_Exception(E::ts('Cannot add more attachments. Maximum limit reached'));
      }

      $remainingAttachments = $maxNumAttachments - $attachmentCount;

      if ($fileCount > $remainingAttachments) {
        throw new CRM_Core_Exception(E::ts('You attempted to add ' . $fileCount . ' attachments, but you can add only ' . $remainingAttachments . ' more attachments'));
      }
      ...
      foreach ($_FILES['files']['name'] as $index => $fileName) {
        ...
        if ($fileSize > ($maxFileSize * 1024 * 1024)) {
          throw new CRM_Core_Exception(E::ts('File size of ' . $fileName . ' exceeds the maximum limit of ' . $fileSize . 'MB'));
        }
      }
      ...
      throw new CRM_Core_Exception(E::ts('Files were not uploaded'));
    }
