<div class="crm-block crm-form-block crm-form-civimobilesettings-block">
    ...
    <div>
        <table class="form-layout-compressed">
            ...
            <tr class="crm-group-form-block-isReserved">
                <td class="label"><label for="civimobile_site_name_to_use">Site name</label> {help id="site-name-help"}
                </td>
                <td>
    ...
    <div>
        <h3>{ts domain="com.agiliway.civimobileapi"}QR-code{/ts}</h3>
    </div>
    <div>
        <table class="form-layout-compressed">
            <tbody>
            ...
            <tr class="crm-group-form-block-isReserved">
                <td>
                    <div>
                        <button id="regenerateQrBtn" class="crm-button default-button" type="button">{ts domain="com.agiliway.civimobileapi"}Regenerate QR Code for Site URL{/ts}</button>
                    </div>
                </td>
            </tr>
            </tbody>
    ...
    <div>
        <table class="form-layout-compressed">
            ...
            {if $form.civimobile_push_notification_reminder_before_task_due}
                <tr class="crm-group-form-block-reminder">
                    <td class="label">
                        {$form.civimobile_push_notification_reminder_before_task_due.label}
                        {help id="reminder-before-due-help"}
                    </td>
                    <td>
                        <div class="crm-reminder-inline">
                            <label>{ts}Days{/ts}</label>
                            {$form.civimobile_push_notification_reminder_before_task_due.days.html}

                            <label>{ts}Hours{/ts}</label>
                            {$form.civimobile_push_notification_reminder_before_task_due.hours.html}

                            <label>{ts}Minutes{/ts}</label>
                            {$form.civimobile_push_notification_reminder_before_task_due.minutes.html}
                        </div>
                    </td>
                </tr>
            {/if}
...
<script>
    ...
    CRM.$(function ($) {
        $('#regenerateQrBtn').click(function () {
            CRM.api4('CiviMobileQrCode', 'update', {}).then(function (res) {
                if (res && res[0].qr_file) {
                    CRM.alert('QR code has been successfully updated!', 'QR Update', 'success');
                } else {
                    CRM.alert('Failed to update QR code.', 'QR Update', 'error');
                }
            });
        });
    });
