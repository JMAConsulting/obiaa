...
class CRM_CiviMobileAPI_Api_CiviMobileParticipantLink_Get extends CRM_CiviMobileAPI_Api_CiviMobileBase {
  ...
  public function getResult() {
    ...
    if ($tmpData = CRM_CiviMobileAPI_BAO_CivimobileEventPaymentInfo::checkByEvent(
      ...
      if (!CRM_CiviMobileAPI_BAO_CivimobileEventPaymentInfo::deleteByHash($tmpData['cmb_hash'])) {
        throw new CRM_Core_Exception(E::ts('Previous Payment cannot be deleted (hash=%1). Please complete payment', [1 => $tmpData['cmb_hash']]), 'participant_previous_payment_not_completed');
      }
    }
    ...
    if ($isSameEmail) {
      throw new CRM_Core_Exception(E::ts('User with same email already registered'), 'participant_with_same_email_exist');
    }
  ...
  protected function getValidParams($params) {
    ...
    if (empty($event)) {
      throw new CRM_Core_Exception(E::ts('Event(id = %1) User can not be registered because event does not exist.', $eventId), 'event_does_not_exist');
    }
    ...
    if ($contactId) {
      ...
      if (!empty($participants)) {
        throw new CRM_Core_Exception(E::ts('Contact(id = %1) is already registered on the Event(id = %2).', [
          1 => $contactId,
          2 => $eventId,
      ...
      if (empty($contact)) {
        throw new CRM_Core_Exception(E::ts('Contact(id = %1) does not exist.', [1 => $contactId]), 'contact_does_not_exist');
      }
    } else {
      if (empty($params['first_name'])) {
        throw new CRM_Core_Exception(E::ts('First name \'%1\' cannot be empty.', [1 => $params['first_name']]), 'contact_has_invalid_first_name');
      }

      if (empty($params['last_name'])) {
        throw new CRM_Core_Exception(E::ts('Last name \'%1\' cannot be empty.', [1 => $params['last_name']]), 'contact_has_invalid_last_name');
      }

      if (!filter_var($params['email'], FILTER_VALIDATE_EMAIL)) {
        throw new CRM_Core_Exception(E::ts('Email \'%1\' have to be valid.', [1 => $params['email']]), 'contact_has_invalid_email');
      }
    }
    ...
    if (!empty($params['price_set'])) {
      ...
      if (empty($priceSetId)) {
        throw new CRM_Core_Exception(E::ts('Can not get price set assigned to event.'), 'event_empty_price_set');
      }
      ...
      if (empty($priceSetFields)) {
        throw new CRM_Core_Exception(E::ts('Can not get price set fields assigned to event.'), 'event_empty_price_set_fields');
      }
  ...
  private function validatePriceSetItems($selPriceSet, $priceSetFields) {
    if (!is_array($selPriceSet)) {
      throw new CRM_Core_Exception(E::ts('Can not parse selected price set'), 'can_not_parse_selected_price_set');
    }
      ...
      foreach ($selPriceSet as $psId => $psFieldIds) {
        foreach ($psFieldIds[0] as $psFieldId => $psFieldValues) {
          if (empty($psFieldValues)) {
            throw new CRM_Core_Exception(E::ts('Can not parse selected value for field (id = %1) for price set (id = %2)', [
              1 => $psFieldId,
              2 => $psId,
            ]), 'field_value_does_not_exist');
          }

          $priceField = $this->findPriceSetFiled($priceSetFields, $psId, $psFieldId);
          if (empty($priceField)) {
            throw new CRM_Core_Exception(E::ts('Price Field (id = %1) does not exist for Event\'s', [1 => $psFieldId]), 'field_id_does_not_exist');
          }

          $priceSetFieldValues = $this->getPriceSetFieldValues($priceField['id']);
          if (empty($priceSetFieldValues)) {
            throw new CRM_Core_Exception(E::ts('Empty filed values for price set field (id = %1). Please create it in administer.', [1 => $priceField['id']]), 'empty_price_set_field_values');
          }

          foreach ($psFieldValues as $item => $psFieldValueId) {
            if ($priceField['html_type'] == 'Text' && empty($psFieldValueId[key($psFieldValueId)])) {
              throw new CRM_Core_Exception(E::ts('"filed_value_count" must be filled for field with "Text" "html_type"'), 'invalid_filed_value_count');
            }

            $priceSetFieldValue = $this->findPriceSetFiledValue($priceSetFieldValues, key($psFieldValueId));
            if (empty($priceSetFieldValue)) {
              throw new CRM_Core_Exception(E::ts('Not valid value(id = %1) for price set field (id = %2).', [
                1 => key($psFieldValueId),
                2 => $psFieldId,
              ]), 'not_valid_value_for_price_set_field');
            }
          }
        }
        ...
        foreach ($priceSetFields as $priceSetField) {
          if ($priceSetField['is_required'] == 1 && !in_array($priceSetField['id'], array_keys($psFieldIds[0]))) {
            throw new CRM_Core_Exception(E::ts('Price field (id = %1) is required field for price set(id = %2)', [
              1 => $priceSetField['id'],
              2 => $priceSetField['price_set_id'],
            ]), 'required_filed_for_price_set');
          }
        }
      }
    }
