  ...
  protected function getValidParams($params) {
    ...
    if (!empty($params['event_id'])) {
      try {
        ...
        throw new CRM_Core_Exception('This event does not exists.', 'event_does_not_exists');
      }
    }
    ...
    if (empty($params['id'])) {
      if (empty($params['event_id'])) {
        throw new CRM_Core_Exception('Missed "event_id" parameter.', 'event_id_is_required');
      }
      if (empty($params['title'])) {
        throw new CRM_Core_Exception('Missed "title" parameter.', 'title_is_required');
      }
      if (empty($params['start_time'])) {
        throw new CRM_Core_Exception('Missed "start_time" parameter.', 'start_time_is_required');
      }
      if (empty($params['end_time'])) {
        throw new CRM_Core_Exception('Missed "end_time" parameter.', 'end_time_is_required');
      }
      if (empty($params['venue_id'])) {
        throw new CRM_Core_Exception('Missed "venue_id" parameter.', 'venue_id_is_required');
      } elseif (!CRM_CiviMobileAPI_Utils_Agenda_Venue::issetVenue($params['venue_id'], $params["event_id"])) {
        throw new CRM_Core_Exception('Venue doesn`t exists.', 'venue_does_not_exists');
      }
      if (!empty($params['speakers']) && !CRM_CiviMobileAPI_Utils_Agenda_Speakers::issetSpeakers($params['speakers'], $params["event_id"])) {
        throw new CRM_Core_Exception('Some speakers does not exists.', 'speakers_does_not_exists');
      }
      ...
      try {
        ...
        throw new CRM_Core_Exception('This session does not exists.', 'session_does_not_exists');
      }
      $sessionId = $session->id;
      if (strlen($params['title']) > 255) {
        throw new CRM_Core_Exception('Title length must be less than 256.', 'title_length_is_too_long');
      }
    ...
    if (!empty($params['speakers']) && CRM_CiviMobileAPI_BAO_EventSession::isSpeakersBusy($params['speakers'], $sessionId, $startTime, $endTime)) {
      throw new CRM_Core_Exception('Some speakers are busy on other Event Session at this time.', 'some_speakers_are_busy');
    }
    if (!empty($params['venue_id']) && CRM_CiviMobileAPI_BAO_EventSession::isVenueBusy($params["venue_id"], $sessionId, $startTime, $endTime)) {
      throw new CRM_Core_Exception('Venue is booked.', 'venues_is_booked');
    }
    $startTime = strtotime($startTime);
    $endTime = strtotime($endTime);
    if ($startTime >= $endTime) {
      throw new CRM_Core_Exception('Start time can`t be later than end time.', 'start_time_cannot_be_later_than_end_time');
    } elseif ($startTime + 15 * 60 > $endTime) {
      throw new CRM_Core_Exception('Event Session duration can not be less than 15 min.', 'event_session_duration_must_be_longer');
    }
    if ($startTime < strtotime($event->start_date)) {
      throw new CRM_Core_Exception('Start time can`t be early than start time on Event.', 'start_time_cannot_be_early_than_start_time_on_event');
    }
    if ($endTime > strtotime($event->end_date) && $event->end_date != NULL) {
      throw new CRM_Core_Exception('End time can`t be later than end time on Event.', 'end_time_cannot_be_later_than_end_time_on_event');
    }
