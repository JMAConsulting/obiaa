...
class CRM_CiviMobileAPI_Api_CiviMobileCmsRegistration_Create extends CRM_CiviMobileAPI_Api_CiviMobileBase {
  ...
  public function getResult() {
    if (!CRM_CiviMobileAPI_Utils_Extension::isAllowCmsRegistration()) {
      throw new CRM_Core_Exception("CiviCRM creating Contact is not allowed", 'creating_contact_error');
    }
    ...
    try {
      ...
      throw new CRM_Core_Exception("CiviCRM creating Contact error: " . $e->getMessage(), 'creating_contact_error');
    }
    ...
    if (!CRM_Core_BAO_CMSUser::create($this->validParams, 'email')) {
      $transaction->rollback();
      throw new CRM_Core_Exception('Creating CiviCRM contact error', 'creating_contact_error');
    }
    ...
    if ($currentCMS == CRM_CiviMobileAPI_Utils_CmsUser::CMS_DRUPAL7 || $currentCMS == CRM_CiviMobileAPI_Utils_CmsUser::CMS_DRUPAL8) {
      if ($currentCMS == CRM_CiviMobileAPI_Utils_CmsUser::CMS_DRUPAL7) {
        ...
        $isEmailVerification = \Drupal::config('user.settings')
          ->get('verify_mail');
        $isAdministratorApproval = \Drupal::config('user.settings')
  ...
  protected function getValidParams($params) {
    ...
    if (!$config->userSystem->isUserRegistrationPermitted()) {
      throw new CRM_Core_Exception("No permissions to create user", 'no_permissions_to_create_user');
    }
    if (strlen($cmsUserParams["name"]) < CMSRegistration::minUsernameLength()) {
      throw new CRM_Core_Exception("Username must have at least " . CMSRegistration::minUsernameLength() . " characters", 'username_has_not_enough_characters');
    }
    if (strlen($cmsUserParams["name"]) > CMSRegistration::maxUsernameLength()) {
      throw new CRM_Core_Exception("Username must have no more " . CMSRegistration::maxUsernameLength() . " characters", 'username_has_too_much_characters');
    }
    if (strlen($cmsUserParams["pass"]) < CMSRegistration::minPasswordLength()) {
      throw new CRM_Core_Exception("Password must have at least " . CMSRegistration::minPasswordLength() . " characters", 'password_has_not_enough_characters');
    }
    if (strlen($cmsUserParams["pass"]) > CMSRegistration::maxPasswordLength()) {
      throw new CRM_Core_Exception("Password must have no more " . CMSRegistration::maxPasswordLength() . " characters", 'password_has_too_much_characters');
    }
    if (preg_match_all("/[a-z]/", $cmsUserParams["pass"]) < CMSRegistration::minPasswordLowerCase()) {
      throw new CRM_Core_Exception("Password must have at least " . CMSRegistration::minPasswordLowerCase() . " characters in lower case", 'password_has_not_enough_characters_in_lower_case');
    }
    if (preg_match_all("/[A-Z]/", $cmsUserParams["pass"]) < CMSRegistration::minPasswordUpperCase()) {
      throw new CRM_Core_Exception("Password must have at least " . CMSRegistration::minPasswordUpperCase() . " characters in upper case", 'password_has_not_enough_characters_in_upper_case');
    }
    if (preg_match_all("/[0-9]/", $cmsUserParams["pass"]) < CMSRegistration::minPasswordIntegers()) {
      throw new CRM_Core_Exception("Password must have at least " . CMSRegistration::minPasswordIntegers() . " integers", 'password_has_not_enough_integers');
    }
    if (preg_match_all("/[^a-zA-Z0-9]/", $cmsUserParams["pass"]) < CMSRegistration::minPasswordSymbols()) {
      throw new CRM_Core_Exception("Password must have at least " . CMSRegistration::minPasswordSymbols() . " symbols", 'password_has_not_enough_symbols');
    }
    if (!preg_match("/^([a-z0-9\+_\-]+)(\.[a-z0-9\+_\-]+)*@([a-z0-9\-]+\.)+[a-z]+$/ix", $cmsUserParams["mail"])) {
      throw new CRM_Core_Exception("Invalid email", 'email_is_not_valid');
    }
    if (strlen($params["first_name"]) > CMSRegistration::maxFirstNameLength()) {
      throw new CRM_Core_Exception("First name must have no more " . CMSRegistration::maxFirstNameLength() . " characters", 'first_name_has_too_much_characters');
    }
    if (strlen($params["last_name"]) > CMSRegistration::maxLastNameLength()) {
      throw new CRM_Core_Exception("Last name must have no more " . CMSRegistration::maxLastNameLength() . " characters", 'last_name_has_too_much_characters');
    }
    ...
    if (!empty($errors)) {
      if (array_key_exists('cms_name', $errors)) {
        throw new CRM_Core_Exception($errors['cms_name'], 'username_already_exists');
      }
      if (array_key_exists('email', $errors)) {
        throw new CRM_Core_Exception($errors['email'], 'email_already_exists');
      }
      throw new CRM_Core_Exception('Unknown error', 'unknown_email_error');
    }
